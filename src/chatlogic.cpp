...
ChatLogic::ChatLogic()
{
    ...
    //_chatBot = new ChatBot("../images/chatbot.png"); no more ownership
...
ChatLogic::~ChatLogic()
{
    ...
    //delete _chatBot; // no more ownership

    // delete all nodes
    // Commenting out the below code as they are unique ptr now
    // should be deleted once out of scope
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // the function does nothing from lifetime point of view, need to only send the raw pointer but how
                        // 
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                           ...
                           // GraphEdge *edge = new GraphEdge(id);
                            ...
                            edge->SetChildNode(childNode->get()); // childnode is the iterator
                            edge->SetParentNode(parentNode->get());
                            // if this is moved it will no longer be available in this context
                            // below line & vector not required
                            ...
                            (*childNode)->AddEdgeToParentNode(edge.get()); // move semantics here
    ...
    // identify root node
    GraphNode *rootNode = nullptr; // change to unique?
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
    ...
    // Useful link - https://knowledge.udacity.com/questions/226150
